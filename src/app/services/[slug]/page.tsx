...
import { ArrowLeft, ArrowRight, ArrowUpRight, Check, ShieldCheck, Sparkles, Clock } from 'lucide-react';
...
export function generateMetadata({ params }: { params: { slug: string } }) {
  ...
  if (!service) {
    return buildMetadata({ title: 'Service', path: '/services' });
  }
  ...
  if (!service) {
    return (
      <Container className="py-20">
        <p className="text-fg-dim">Service not found.</p>
        <Link className="mt-4 inline-flex items-center gap-1.5 text-sm text-fg hover:text-brand-500" href="/services">
    ...
    <Container className="py-16 sm:py-20">
      ...
      <header className="mt-8 max-w-3xl">
        ...
        <div className="mt-7 flex flex-wrap items-center gap-x-6 gap-y-2 text-sm text-fg-dim">
          ...
          <span className="inline-flex items-center gap-1.5"><Sparkles size={13} /> Starts at {service.startsAt}</span>
        </div>
        ...
        <div className="mt-8 grid gap-5 md:grid-cols-3">
          {service.pricing.map((tier) => (
            ...
            >
              ...
              <div className="font-display text-2xl text-fg numeric">{tier.price}</div>
